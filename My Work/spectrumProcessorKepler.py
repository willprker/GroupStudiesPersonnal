...
class PowerSpectrumProcessor:
    ...
    def process_filter(self, spacing, queue):
        ''' Process the filtering operation for a given spacing and add the result to the queue '''
        if self.signal is None:
            raise ValueError("Signal data is not loaded.")

        x = np.linspace(-70000, 70000, 140000)
        gamma = 200
        a = 10
        ...
        output = np.convolve(self.signal[10000:700000], combined_filter, mode='valid')

        start_index = 150000
        end_index = start_index + len(output)
        output_frequencies = self.frequencies[start_index:end_index]

        queue.put((spacing, output, output_frequencies))

    def run_filtering(self):
        ''' Run the filtering operation for different spacings '''
        if self.frequencies is None or self.signal is None:
            raise ValueError("Data not loaded. Run load_power_spectrum() first.")

        spacing_points = np.arange(49, 100, 1)
        spacing_microhz = spacing_points * 99.14667583219213
        ...
        for _ in range(len(processes)):
            spacing, output, output_frequencies = queue.get()
    ...
    def save_results(self, subtract_baseline=False):
        ...
        for spacing, (output, output_frequencies) in self.results.items():
            frequency_label = f"{(spacing * 2) / 99.14667583219213} microHz"
        ...
        output_filename = f"{filename}_filter_data.csv"
    ...
    def plot_filtered_outputs(self, subtract_baseline=False):
        ...
        for ax, (spacing, (output, output_frequencies)) in zip(axes, self.results.items()):
            frequency = (spacing*2)/99.14667583219213
